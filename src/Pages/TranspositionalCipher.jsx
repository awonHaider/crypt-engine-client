  ...
  const handleEncrypt = async () => {
    ...
    try {
      const response = await fetch(
        "http://localhost:8000/api/v1/encrypt/transpositional-cipher",
        {
          method: "POST",
          headers: {
      ...
      setError(
        err.message || "Key must be a permutation of digits (e.g., '4312567')"
      );
    } finally {
      setLoading(false);
    }
  ...
  return (
    <>
      <div className="api-section pl-[2rem] mb-[2rem] md:pl-[8rem] md:pt-[5rem]">
        <h3 className="mb-6 text-xl font-bold">Transpositional Cipher</h3>
        ...
        >
          <input
            className="text-sm w-full bg-transparent outline-none"
            placeholder="Enter key (digits only, e.g., '4312567')"
            type="text"
            value={key}
            onChange={(e) => setKey(e.target.value.replace(/\D/g, ""))} // Remove non-digits
          />
        </div>
...
export default TranspositionalCipher;
